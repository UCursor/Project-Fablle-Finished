...
const DEFAULTS: Required<LiquidOptions> = {
  ...
  distortion: 0.4,
  blend: 5,
  color: [1, 1, 1], // Pure white for perfect color inversion via mix-blend-mode: difference
  rainbow: false,
};
...
void main () {
  ...
  if (uHasContent < 0.5) {
    ...
    vec3 tint = uRainbow == 1.0
      ...
      : uColor;
    // Crisp, sharp liquid silhouette without soft blur
    float overlay = smoothstep(0.015, 0.03, mag * uIntensity);
    outColor = vec4(tint * overlay, overlay);
    return;
  }
...
export function createLiquid(
  ...
  function onPointerMove(event: PointerEvent) {
    ...
    if (px < -100 || px > rect.width + 100 || py < -100 || py > rect.height + 100) {
    ...
    queued.push([
      Math.max(0, Math.min(1, px / rect.width)),
      Math.max(0, Math.min(1, 1 - py / rect.height)),
      dx,
      dy,
    ]);
    start();
...
export function Liquid({
  ...
  return (
    <div className={`liquid-container ${className || ''}`} style={{ position: "relative", ...style }}>
      <canvas
        ref={sourceRef}
        {...({ layoutsubtree: "true" } as Record<string, string>)}
      ...
      <canvas
        ...
        style={{
          ...
          pointerEvents: "none",
          zIndex: 10,
          mixBlendMode: invertMask ? "difference" : "normal",
        }}
      />
    </div>
  );
